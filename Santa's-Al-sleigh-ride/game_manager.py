...
class GameManager:
    ...
    def display_states(self, states_list: list[State]) -> None:
        ...
        self.display.begin_display(len(states_list))
    ...
    def bd_bfs_search(self) -> (list[Node], int, int):
        ...
        new_gifts = self.init_state.gifts.copy()                # Putting gifts into points to create goal state
        for i, point in enumerate(self.map.points):
            new_gifts[i] = point

        all_goal_states = []                            # Putting the santa in all possible positions around gift
        for i, point in enumerate(self.map.points):
            for direction in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
                ...
                # Checking gifts around gift
                if (new_y, new_x) in new_gifts:
                    continue

                state = State((new_y, new_x), new_gifts.copy())
    ...
    def ucs_search(self) -> Node:
        """ Performs a Uniform Cost Search from initial state to goal state.
        ...
        # Set initial state
        root_node = Node(self.init_state)
        priority_queue.put((0, root_node))  # Priority is the path cost
        ...
        while not priority_queue.empty():
            ...
            for child in node.expand(actions):
                priority_queue.put((child.path_cost, child))
    ...
    def parse_map() -> (Map, State):
        ...
        gifts = []                                            # Variables to read from map
        points = []
        santa = (0, 0)
        ...
        return map_object, State(santa, gifts)
